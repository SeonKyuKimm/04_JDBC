...
public class MemberView {
	...
	public boolean unRegisterMenu() {
		...
		System.out.printf("보안 문자 입력 [%s] : ", code); // 보안 문자 입력 [240319] : 대괄호 안에꺼 치면 됨
		String input = sc.next();// 보안 문자 입력
		
		// 보안 문자 일치 여부 확인하기
		if( !input.equals(code)) { // 일치하지 않으면 이라는 뜻 ㅎ;
			System.out.println("\n***보안문자가 일치하지 않습니다***\n");
			return false;
		}
		while(true) {
			...
			if(check == 'N') {
				...
				System.out.println("\n---탈퇴 취소 ㄳㄳ---\n");
				return false; // 해당 메서드 종료, false 값 메뉴로 반환
			}
			if(check == 'Y') {
				break; // 반복문 종료
