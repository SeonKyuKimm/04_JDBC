...
public class CommentView {
	...
	public void updateComment(int boardNo) {
		...
		try {
			...
			// 입력할 댓글 번호가 해당 게시글에 대해 로그인한 회원이 작성한 댓글이 맞는지 확인
			int check = service.checkCommentNo(commentNo, boardNo, Session.loginMember.getMemberNo() );
	...
	private void insertComment(int boardNo) {
		System.out.println("[댓글 입력] <!wq입력시 종료>");
		
		// String : 불변성 
		// StringBuffer : 가변성 (메모리에 객체 하나 만들어지면 이어쓰기, 변경사항 등 하나의 객체만 이용함 )
