...
public class BoardView {
	...
	public void selectBoard() {
		...
		// -> 조회수가 증가( 단, 자신이 작성한 게시글일 경우 조회수 증가 X )
		...
		try {
			...
			Board board = boardService.selectBoard(input, Session.loginMember.getMemberNo() );
										// 입력받은 게시글 번호, 로그인한 회원번호( 조회수 증가용으로 사용)
				...
				while(true) {
					...
					switch(input) {
						...
						case 1 : updateBoard( board.getBoardNo() ); return;
								 // 게시글 번호를 매개변수로 전달해서 해당 번호의 게시글을 수정
						
						case 2 : deleteBoard( board.getBoardNo() ); return;
								 // 게시글 번호를 매개변수로 전달해서 해당 번호의 게시글을 삭제
